...
function TopFit:CreateProgressFrame()
    if not TopFit.ProgressFrame then
        ...
        TopFit.ProgressFrame.importExportButton = CreateFrame("Button", "TopFit_ProgressFrame_importExportButton", TopFit.ProgressFrame)
        TopFit.ProgressFrame.importExportButton:SetPoint("LEFT", TopFit.ProgressFrame.addSetButton, "RIGHT", 4, 0)
        TopFit.ProgressFrame.importExportButton:SetHeight(12)
        TopFit.ProgressFrame.importExportButton:SetWidth(12)
        TopFit.ProgressFrame.importExportButton:SetNormalTexture("Interface\\Icons\\INV_Pet_RatCage")
        TopFit.ProgressFrame.importExportButton:SetHighlightTexture("Interface\\Buttons\\ButtonHilight-Square")

        TopFit.ProgressFrame.importExportButton.menu = CreateFrame("Frame", "TopFit_ProgressFrame_importExportButton_menu", TopFit.ProgressFrame.importExportButton, "UIDropDownMenuTemplate")
        UIDropDownMenu_Initialize(TopFit.ProgressFrame.importExportButton.menu, function(self, level)
            local info = UIDropDownMenu_CreateInfo()
            info.hasArrow = false
            info.notCheckable = true
            info.text = "Import set from string..."
            info.func = function() TopFit:ShowImportDialog() end
            UIDropDownMenu_AddButton(info, level)

            info = UIDropDownMenu_CreateInfo()
            info.hasArrow = false
            info.notCheckable = true
            info.text = "Export selected set (TopFit)"
            info.func = function() TopFit:ShowExportDialog(false) end
            UIDropDownMenu_AddButton(info, level)

            info = UIDropDownMenu_CreateInfo()
            info.hasArrow = false
            info.notCheckable = true
            info.text = "Export selected set (Pawn)"
            info.func = function() TopFit:ShowExportDialog(true) end
            UIDropDownMenu_AddButton(info, level)

            info = UIDropDownMenu_CreateInfo()
            info.hasArrow = false
            info.notCheckable = true
            info.text = "Export equipped gear (SimC)"
            info.func = function() TopFit:ShowSimcExportDialog() end
            UIDropDownMenu_AddButton(info, level)
        end, "MENU")

        TopFit.ProgressFrame.importExportButton:SetScript("OnClick", function(self)
            ToggleDropDownMenu(1, nil, self.menu, self, 0, 0)
        end)
        TopFit.ProgressFrame.importExportButton.tipText = "Import or export weight sets\n|cffffffffImport Pawn/AskMrRobot strings, or export your own sets to share."
        TopFit.ProgressFrame.importExportButton:SetScript("OnEnter", ShowTooltip)
        TopFit.ProgressFrame.importExportButton:SetScript("OnLeave", HideTooltip)
